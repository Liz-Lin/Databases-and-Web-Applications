...
function handleLookup(query, doneCallback) {
    ...
    if (query in cachedQueryResult){
        console.log("found query in cache");
        var data = cachedQueryResult[query];
        handleLookupAjaxSuccess(data, query, doneCallback, 1);
    }
    else {
        ...
        jQuery.ajax({
            ...
            "url": "/api/movieAutoComplete?query=" + escape(query),
            "success": function (data) {
                // pass the data, query, and doneCallback function into the success handler
                handleLookupAjaxSuccess(data, query, doneCallback, 0)
            },
 ...
 *
 */
function handleLookupAjaxSuccess(data, query, doneCallback, isFromCache) {
    ...
    if (!isFromCache){
        ...
        cachedQueryResult[query] = jsonData;
    }
    else
    {
        jsonData=data;
...
function handleNormalSearch(query) {
    ...
    var url = "/list.html?MovieTitle="+query+"&orderBy=title&is_androd=0";
    window.location.replace(url);
...
$(document).ready(function() {
    ...
    $('#autocomplete').keypress(function(event) {
        ...
        if (event.keyCode == 13) {
            console.log("press enter key, go to normal search");
    ...
    $('#autocomplete-button').click(function (event) {
        console.log("press search button, go to normal search");
        event.preventDefault();
        handleNormalSearch($('#autocomplete').val());
    });
})